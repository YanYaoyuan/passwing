...
// 建立 SSE 长连接
// ─────────────────────────────────────────────────────────────────
void jobManager::startFun3dSSE(const QString &jobId)
{
    stopFun3dSSE();   // 先关闭旧连接

    QUrl url(QString("http://112.46.144.60:8000/fun3d/stream/%1").arg(jobId));
    QNetworkRequest request(url);
...
void jobManager::onSseLine(const QString &line)
{
    ...
    if (s_event == "iteration") {
        ...
        double densityMax = data.value("density_max").toDouble();
