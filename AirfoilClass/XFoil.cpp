...
void XFoil::hipnt(double chpnt, double thpnt) {
  ...
  maxThickness = tymax * 2;
  maxCamber = cymax;
  locationThickness = txmax;
  locationCamber = cxmax;
...
bool XFoil::setbl() {
  ...
  jvte1 = int(isys[iblte[1]][1]);
  jvte2 = int(isys[iblte[2]][2]);
...
bool XFoil::eiwset(int nc1) {
  ...
  //---- set  m = 1  numbers
  for (ic = 1; ic <= nc; ic++) eiw[ic][1] = exp(std::complex<double>(0.0, wc[ic]));
...
void XFoil::cgauss(int nn, std::complex<double> z[IMX4 + 1][IMX4 + 1],
