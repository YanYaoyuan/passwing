    ...
    if(modelType == AIRFOIL_PROFILI){
        if(isSort){
            ...
            if (!selectedIndexes.isEmpty()) {
                ...
                // 从模型中获取对应的文本
                QString selectedText = listModel->data(index, Qt::DisplayRole).toString();
...
void airfoilLibary::sortData(QVector<QVector<double>>& data, int column, bool descending) {
    // 创建比较函数
...
QVector<QVector<double>> airfoilLibary::readCST()
{
    ...
    try {
        H5::DataSet ds = file->openDataSet("/shape/cst");
        ...
        data.resize(nAirfoils); // 1830
        for(int p = 0; p < nAirfoils; ++p){
            data[p].resize(nParams); // 19
            for(int a = 0; a < nParams; ++a){
                data[p][a] = raw[p * nParams + a]; // ✅ 正确
            }
        }
        ...
        for (int a = 0; a < nAirfoils; ++a) {
            QString names = QString("windAI%1").arg(a, 4, 10, QChar('0'));
            AirfoilC[a].Name = names;
            airfoilNameArrayC.append(names);
        }
