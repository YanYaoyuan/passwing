...
void airfoilAnalyse::initialSetting(const QVector<double>&setting){
    if(setting.isEmpty()){
        ...
        MINAXISR[0] = setting[0];    MAXAXISR[0] = setting[8];
        MINAXISR[1] = setting[1];    MAXAXISR[1] = setting[9];
        MINAXISR[2] = setting[2];    MAXAXISR[2] = setting[10];
        MINAXISR[3] = setting[3];    MAXAXISR[3] = setting[11];
        MINAXISR[4] = setting[4];    MAXAXISR[4] = setting[12];
        MINAXISR[5] = setting[5];    MAXAXISR[5] = setting[13];
        MINAXISR[6] = setting[6];    MAXAXISR[6] = setting[14];
        MINAXISR[7] = setting[7];    MAXAXISR[7] = setting[15];
    }
...
void airfoilAnalyse::startXfoil(const int n){
    ...
    if(file.open(QIODevice::Append|QIODevice::Text)){
        ...
        for(const QVector <double>&data:all){
            stream<<AirfoilB[threadNum * n + i].Name<<"    ";
            for(const double da:data){
                stream<<da<<"    ";
            }
            stream<<"\n";
            i++;
