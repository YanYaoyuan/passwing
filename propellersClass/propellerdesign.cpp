...
void propellerDesign::twistProp(){
    ...
    for(int i = 0;i<num;i++){
        for(int j = 0;j<meshY;j++){
            int G = i * meshY + j;
            double k = (propTwist[num - i - 1] - propTwist[num - i]) /(damiterArray[num - i] - damiterArray[num - i - 1]);
            double t = (propTwist[num - i] + k * (yt[G] + damiterArray[num - i])) / 180 * M_PI;
            th.append(t);
        }
    }
...
point3d propellerDesign::rotatePointAroundAxis(const point3d& point, const point3d& axisPoint, const point3d& axisDir, double angle) {
    // 将角度转换为弧度
    ...
    const QVector<double>& kp)
{
    ...
    for (int i = 0; i < n - 1; ++i) {
        ...
        double f1 = (K1_1 + K1_1 * K1_1) * kp[i] * r1;

        double K1_2 = K / (1.0 + pow(V / (omega * r2), 2.0) * pow(1.0 + K, 2.0));
        double f2 = (K1_2 + K1_2 * K1_2) * kp[i+1] * r2;
