...
void wingVLM::computeAerodynamicCoefficient(double area,double mac){
...
void wingVLM::startXfoilInThread(const int index,const int num){
...
void wingVLM::computeForce(const QVector<QVector<point3d>>&tmp){
    streamVinfArray = tmp;
...
void wingVLM::solveStreamLine(const int index, const double distance,const double height,const double t,const double dt){
    streamIndex = index;
...
void wingVLM::solveSpanForce(const QVector<QVector<double> > &array,double rArea){
...
QVector<double> wingVLM::calculateBendingMoment(const QVector<double>& spanForce, const QVector<double>& yt) {
    QVector<double>tmp;
    int len = yt.length() / 2;
    for(int i = 0;i<len;i++){
        double tmp1 = 0;
