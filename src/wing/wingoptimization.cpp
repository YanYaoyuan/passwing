...
void wingOptimization::initialElite(){
    ...
    int cstMax, cstMin;
    double cst,span,chord,generatedOffset,twisAngle,dihedralAngle;
        ...
        for(int j = 0;j<numN;j++){
           ...
           // span = generateRandomInt(int(initialWingData.spanW[j] * minRatio / valueList.spanVal),int(initialWingData.spanW[j] * maxRatio / valueList.spanVal)) * valueList.spanVal;
            ...
            if(valueList.offsetChangeArray[j]){
                generatedOffset = generateRandomInt(int(valueList.minOffsetLength[j] / valueList.offsetLengthVal),int(valueList.maxOffsetLength[j] / valueList.offsetLengthVal)) * valueList.offsetLengthVal;
                defineTmp.offsetLengthW.append(generatedOffset);
                // if(offsetLength != 0)
                //     symbol1.append(offsetLength / abs(offsetLength));
                // else
                //     symbol1.append(1);
                symbol1.append(getSign(generatedOffset));
            }else{
...
void wingOptimization::changeWingData(wingDefinition& wingData){
    ...
    for(int j = 0;j<numN;j++){
        ...
        if(valueList.offsetChangeArray[j]){
            generatedOffset = generateRandomInt(int(valueList.minOffsetLength[j] / valueList.offsetLengthVal),int(valueList.maxOffsetLength[j] / valueList.offsetLengthVal)) * valueList.offsetLengthVal;
            offsetArrayA.append(generatedOffset);
        }else{
