...
void cfdDisplay::showFlowModelSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
...
void cfdDisplay::showVolumeOutputSetting(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showBoundaryOutputSetting(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showReferenceSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showEquationSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showForceMomentSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showRawGridSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showInviscidFluxSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
*/
void cfdDisplay::showCodeRunControlSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showNonlinearSolverSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showLinearSolverSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::showAirfoil(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
...
void cfdDisplay::showCloudPlotSetup(QTreeWidgetItem*item,int /*column*/){
    if(item->parent()){
        QString txt1 = item->data(0,Qt::UserRole).toString();
...
void cfdDisplay::handleIterationData(const IterationData &iterationData)
{
     if (!updatingPlots) return;
     if (iterationData.iter <= 0) return;
    int iter = iterationData.iter;

    // ===============================
    // 📉 残差图数据更新（plotViewA）
    // ===============================
    if (densityRMSGraph) densityRMSGraph->addData(iter, iterationData.densityRMS);
    if (densityMAXGraph) densityMAXGraph->addData(iter, iterationData.densityMAX);
    if (densityXGraph)   densityXGraph->addData(iter, iterationData.densityX);
    if (densityYGraph)   densityYGraph->addData(iter, iterationData.densityY);
    if (densityZGraph)   densityZGraph->addData(iter, iterationData.densityZ);

    if (turbRMSGraph) turbRMSGraph->addData(iter, iterationData.turbRMS);
    if (turbMAXGraph) turbMAXGraph->addData(iter, iterationData.turbMAX);
    if (turbXGraph)   turbXGraph->addData(iter, iterationData.turbX);
    if (turbYGraph)   turbYGraph->addData(iter, iterationData.turbY);
    if (turbZGraph)   turbZGraph->addData(iter, iterationData.turbZ);

    // ===============================
    // ✈️ 升阻力图数据更新（plotViewB）
    // ===============================
    if (liftGraph) liftGraph->addData(iter, iterationData.lift);
    if (dragGraph) dragGraph->addData(iter, iterationData.drag);
...
void cfdDisplay::updateAirfoilArray(const QVector<QVector<QVector<double>>>&airfoilArrayTmp,const QVector<QString>&incomingNames,const QVector<int>&cstNumArrayTmp){

    if(!airfoilArrayTmp.isEmpty() && !incomingNames.isEmpty()){

        QStringList boxList;
        for(int i = 0;i<incomingNames.length();i++){
            boxList.append(incomingNames[i]);
        }

        updateCFDTree(airfoilRoots,incomingNames);
        ...
        */
        ...
        AirfoilNameArray = incomingNames;
