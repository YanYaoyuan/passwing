...
void flowViewer::loadFlattenedGridAuto(const QString& filePath)
{
    ...
    if ((int)coordinates.size() < 3 * N)
    {
        qDebug() << "❌ 数据不足，期望:" << 3 * N << "实际:" << coordinates.size();
        return;
    }
    ...
    // 3️⃣ 拆分 XYZ
    // =========================
    std::vector<double> X(coordinates.begin(), coordinates.begin() + N);
    std::vector<double> Y(coordinates.begin() + N, coordinates.begin() + 2 * N);
    std::vector<double> Z(coordinates.begin() + 2 * N, coordinates.begin() + 3 * N);
