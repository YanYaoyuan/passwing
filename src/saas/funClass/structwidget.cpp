...
void structWidget::updateGlobalBox(const structCFDDefinition &def)
{
    ...
    const auto &settingsData = def.globalData; // 假设 structDefinition 有这个成员
    ...
    // ===== 设置状态 =====
    globalUI.movingGridCheck->setChecked(settingsData.movingGrid.toLower() == ".true." || settingsData.movingGrid == "1");
    globalUI.gridMotionOnlyCheck->setChecked(settingsData.gridMotionOnly.toLower() == ".true." || settingsData.gridMotionOnly == "1");
    globalUI.gridMotionAndDCICheck->setChecked(settingsData.gridMotionAndDCI.toLower() == ".true." || settingsData.gridMotionAndDCI == "1");
    globalUI.bodyMotionOnlyCheck->setChecked(settingsData.bodyMotionOnly.toLower() == ".true." || settingsData.bodyMotionOnly == "1");

    globalUI.timingCheck->setChecked(settingsData.timing.toLower() == ".true." || settingsData.timing == "1");
    globalUI.timeMovingGridCheck->setChecked(settingsData.timeMovingGrid.toLower() == ".true." || settingsData.timeMovingGrid == "1");

    globalUI.boundaryAnimSpin->setValue(settingsData.boundaryAnimFreq);
    globalUI.volumeAnimSpin->setValue(settingsData.volumeAnimFreq);
    globalUI.sliceFreqSpin->setValue(settingsData.sliceFreq);

    globalUI.recordCmdCheck->setChecked(settingsData.recordCmd.toLower() == ".true." || settingsData.recordCmd == "1");
}

void structWidget::updateTurbulentDiffusionBox(const structCFDDefinition &def)
{
    const auto &settingsData = def.turbulentDiffusionModelsData; // 假设你在 structDefinition 里有这个成员
    ...
    // === 湍流模型选择 ===
    QString modelNorm = settingsData.turbulenceModel.toLower();
    ...
    // === 参数更新 ===
    flowUI.turbIntensitySpin->setValue(settingsData.turbIntensity);
    flowUI.turbMuRatioSpin->setValue(settingsData.turbMuRatio);
    flowUI.turbCompressCombo->setCurrentText(settingsData.turbCompress);
    flowUI.turbCondCombo->setCurrentText(settingsData.turbCond);
    flowUI.prSpin->setValue(settingsData.pr);
    flowUI.scSpin->setValue(settingsData.sc);

    // === 同步联动状态 ===
    bool compressible = (settingsData.turbCompress.compare("on", Qt::CaseInsensitive) == 0);
...
{
    const auto &settingsData = def.eqnData; // 假设 structDefinition 里有 EqnData eqnData;
    ...
    // === 同步 UI 值 ===
    eqnUI.eqnTypeCombo->setCurrentText(settingsData.eqnType);
    eqnUI.artificialCompressSpin->setValue(settingsData.artificialCompress);
    eqnUI.viscousTermsCombo->setCurrentText(settingsData.viscousTerms);
    eqnUI.chemKineticsCombo->setCurrentText(settingsData.chemicalKinetics);
    eqnUI.thermalModelCombo->setCurrentText(settingsData.thermalEnergyModel);
    eqnUI.prandtlSpin->setValue(settingsData.prandtlNumberMolecular);
    eqnUI.schmidtSpin->setValue(settingsData.schmidtNumber);
    eqnUI.gasRadiationCombo->setCurrentText(settingsData.gasRadiation);
    //eqnUI.cpivMinSpin->setValue(settingsData.cpivMinFactor);

    eqnUI.radUseImplCheck->setChecked(settingsData.radUseImplLines.compare(".true.", Qt::CaseInsensitive) == 0);
    eqnUI.multiDiffCheck->setChecked(settingsData.multiComponentDiff.compare(".true.", Qt::CaseInsensitive) == 0);
    //eqnUI.augmentKineticsCheck->setChecked(settingsData.augmentKineticsLimiting.compare(".true.", Qt::CaseInsensitive) == 0);
    //eqnUI.implicitRateCheck->setChecked(settingsData.implicitRateLimiting.compare(".true.", Qt::CaseInsensitive) == 0);

    // === 手动刷新联动状态 ===
    bool isIncompressible = (settingsData.eqnType == "incompressible");
    bool isGeneric = (settingsData.eqnType == "cal_perf_compress");
...
{
    const CodeRunData &settingsData = def.codeRunData;
    ...
    // === 基本控制 ===
    codeRunUI.stepsSpin->setValue(settingsData.steps);
    codeRunUI.stopTolEdit->setText(QString::number(settingsData.stoppingTolerance));
    //codeRunUI.stopTolSpin->setValue(settingsData.stoppingTolerance);
    codeRunUI.durationLimitSpin->setValue(settingsData.durationLimit);

    // === 重启控制 ===

    codeRunUI.noRestartCheck->setChecked(settingsData.noRestart == ".true.");
    codeRunUI.restartWriteFreqSpin->setValue(settingsData.restartWriteFreq);
    //
    QString restartReadValue = cleanQuotedText(settingsData.restartRead);  // 去掉首尾空格
    ...
    // === Jacobian 更新策略 ===
    codeRunUI.smartJupdateCheck->setChecked(settingsData.smartJupdate == ".true.");
    codeRunUI.jacobianEvalFreqSpin->setValue(settingsData.jacobianEvalFreq);
    codeRunUI.jupdateStartupStepsSpin->setValue(settingsData.jupdateStartupSteps);
    codeRunUI.dfduc3Check->setChecked(settingsData.dfduc3Jacobians == ".true.");

    // === 攻角扫描 ===
    codeRunUI.alphaSweepCheck->setChecked(settingsData.alphaSweep == ".true.");
    codeRunUI.cycleIncrementSpin->setValue(settingsData.cycleIncrement);
    codeRunUI.alphaIncrementSpin->setValue(settingsData.alphaIncrement);
    codeRunUI.alphaMaxSpin->setValue(settingsData.alphaMax);
    codeRunUI.alphaMinSpin->setValue(settingsData.alphaMin);
    codeRunUI.alphaSwitchbacksSpin->setValue(settingsData.alphaSwitchbacks);
...
{
    const NonlinearSolverData &settingsData = def.nonlinearSolverData;
    ...
    // ===== time_accuracy =====
    QString timeAcc = cleanQuotedText(settingsData.timeAccuracy);
    ...
    QString pseudo = cleanQuotedText(settingsData.pseudoTimeStepping);
    ...
    // ===== 其它参数 =====
    nonlinearUI.timeStepNondimSpin->setValue(settingsData.timeStepNondim);
    nonlinearUI.subiterationsSpin->setValue(settingsData.subiterations);
    nonlinearUI.temporalErrControlCheck->setChecked(settingsData.temporalErrControl == ".true.");
    nonlinearUI.temporalErrFloorSpin->setValue(settingsData.temporalErrFloor);
    nonlinearUI.scheduleNumberSpin->setValue(settingsData.scheduleNumber);
    nonlinearUI.scheduleIteration1Spin->setValue(settingsData.scheduleIteration1);
    nonlinearUI.scheduleIteration2Spin->setValue(settingsData.scheduleIteration2);
    nonlinearUI.scheduleCfl1Spin->setValue(settingsData.scheduleCfl1);
    nonlinearUI.scheduleCfl2Spin->setValue(settingsData.scheduleCfl2);
    nonlinearUI.scheduleCflturb1Spin->setValue(settingsData.scheduleCflturb1);
    nonlinearUI.scheduleCflturb2Spin->setValue(settingsData.scheduleCflturb2);
    nonlinearUI.fAllowMinimumMSpin->setValue(settingsData.fAllowMinimumM);
    nonlinearUI.inviscRelaxFactorSpin->setValue(settingsData.inviscRelaxFactor);
    nonlinearUI.viscRelaxFactorSpin->setValue(settingsData.viscRelaxFactor);
...
{
    const LinearSolverData &settingsData = def.linearSolverData;

    // === 主流方程松弛次数 ===
    linearSolverUI.meanflowSweepsSpin->setValue(settingsData.meanflowSweeps);

    // === 湍流方程松弛次数 ===
    linearSolverUI.turbSweepsSpin->setValue(settingsData.turbulenceSweeps);

    // === Krylov 投影 ===
    linearSolverUI.projectionCheck->setChecked(settingsData.linearProjection == ".true.");

    // === 线隐式选项 ===
    QString lineImp = cleanQuotedText(settingsData.lineImplicit);
...
{
    const VolumeOutputData &settingsData = def.volumeOutputData;

    // 🟡 一次性屏蔽所有信号
    const bool oldState = blockSignals(true);

    // === 输出格式 export_to ===
    QString exportTo = cleanQuotedText(settingsData.exportTo);
    ...
    // === 坐标 ===
    volumeOutputUI.xCheck->setChecked(settingsData.x == ".true.");
    volumeOutputUI.yCheck->setChecked(settingsData.y == ".true.");
    volumeOutputUI.zCheck->setChecked(settingsData.z == ".true.");

    // === 基础变量 ===
    volumeOutputUI.primitiveCheck->setChecked(settingsData.primitive == ".true.");
    volumeOutputUI.rhoCheck->setChecked(settingsData.rho == ".true.");
    volumeOutputUI.uCheck->setChecked(settingsData.u == ".true.");
    volumeOutputUI.vCheck->setChecked(settingsData.v == ".true.");
    volumeOutputUI.wCheck->setChecked(settingsData.w == ".true.");
    volumeOutputUI.pCheck->setChecked(settingsData.p == ".true.");
    volumeOutputUI.entropyCheck->setChecked(settingsData.entropy == ".true.");
    volumeOutputUI.machCheck->setChecked(settingsData.mach == ".true.");
    volumeOutputUI.temperatureCheck->setChecked(settingsData.temperature == ".true.");
    volumeOutputUI.iblankCheck->setChecked(settingsData.iblank == ".true.");
    volumeOutputUI.imeshCheck->setChecked(settingsData.imesh == ".true.");

    // === 涡量/速度散度 ===
    volumeOutputUI.vortMagCheck->setChecked(settingsData.vortMag == ".true.");
    volumeOutputUI.vortXCheck->setChecked(settingsData.vortX == ".true.");
    volumeOutputUI.vortYCheck->setChecked(settingsData.vortY == ".true.");
    volumeOutputUI.vortZCheck->setChecked(settingsData.vortZ == ".true.");
    volumeOutputUI.qCriterionCheck->setChecked(settingsData.qCriterion == ".true.");
    volumeOutputUI.divVelCheck->setChecked(settingsData.divVel == ".true.");

    // === 湍流波动 ===
    volumeOutputUI.turbFluctCheck->setChecked(settingsData.turbFluct == ".true.");
    volumeOutputUI.uuPrimeCheck->setChecked(settingsData.uuPrime == ".true.");
    volumeOutputUI.vvPrimeCheck->setChecked(settingsData.vvPrime == ".true.");
    volumeOutputUI.wwPrimeCheck->setChecked(settingsData.wwPrime == ".true.");
...
{
    const BoundaryOutputData &settingsData = def.boundaryOutputData;
    ...
    // === 基本参数 ===
    boundaryOutputUI.numberOfBoundaries->setValue(settingsData.numberOfBoundaries);

    QString list = settingsData.boundaryList;
    list.remove("'"); // 移除包裹的引号
    ...
    // === 坐标 ===
    boundaryOutputUI.xCheck->setChecked(settingsData.x == ".true.");
    boundaryOutputUI.yCheck->setChecked(settingsData.y == ".true.");
    boundaryOutputUI.zCheck->setChecked(settingsData.z == ".true.");

    // === 基础变量 ===
    boundaryOutputUI.primitiveCheck->setChecked(settingsData.primitive == ".true.");
    boundaryOutputUI.rhoCheck->setChecked(settingsData.rho == ".true.");
    boundaryOutputUI.uCheck->setChecked(settingsData.u == ".true.");
    boundaryOutputUI.vCheck->setChecked(settingsData.v == ".true.");
    boundaryOutputUI.wCheck->setChecked(settingsData.w == ".true.");
    boundaryOutputUI.pCheck->setChecked(settingsData.p == ".true.");
    boundaryOutputUI.entropyCheck->setChecked(settingsData.entropy == ".true.");
    boundaryOutputUI.machCheck->setChecked(settingsData.mach == ".true.");
    boundaryOutputUI.temperatureCheck->setChecked(settingsData.temperature == ".true.");
    boundaryOutputUI.iblankCheck->setChecked(settingsData.iblank == ".true.");
    boundaryOutputUI.imeshCheck->setChecked(settingsData.imesh == ".true.");

    // === 涡量/散度 ===
    boundaryOutputUI.vortMagCheck->setChecked(settingsData.vortMag == ".true.");
    boundaryOutputUI.vortXCheck->setChecked(settingsData.vortX == ".true.");
    boundaryOutputUI.vortYCheck->setChecked(settingsData.vortY == ".true.");
    boundaryOutputUI.vortZCheck->setChecked(settingsData.vortZ == ".true.");
    boundaryOutputUI.qCriterionCheck->setChecked(settingsData.qCriterion == ".true.");
    boundaryOutputUI.divVelCheck->setChecked(settingsData.divVel == ".true.");

    // === 湍流相关 ===
    boundaryOutputUI.turbFluctCheck->setChecked(settingsData.turbFluct == ".true.");
    boundaryOutputUI.uuPrimeCheck->setChecked(settingsData.uuPrime == ".true.");
    boundaryOutputUI.vvPrimeCheck->setChecked(settingsData.vvPrime == ".true.");
    boundaryOutputUI.wwPrimeCheck->setChecked(settingsData.wwPrime == ".true.");
    boundaryOutputUI.uvPrimeCheck->setChecked(settingsData.uvPrime == ".true.");
    boundaryOutputUI.uwPrimeCheck->setChecked(settingsData.uwPrime == ".true.");
    boundaryOutputUI.vwPrimeCheck->setChecked(settingsData.vwPrime == ".true.");
    boundaryOutputUI.cpCheck->setChecked(settingsData.cp == ".true.");
}


void structWidget::updateReferenceBox(const structCFDDefinition &def)
{
    const auto &settingsData = def.referenceData;  // 假设 structDefinition 里有 ReferenceData referenceData;
    ...
    // === 基础值同步 ===
    refUI.gridLengthConversionSpin->setValue(settingsData.gridlengthConversion);
    refUI.machSpin->setValue(settingsData.mach);
    refUI.reynoldsSpin->setValue(settingsData.reynolds);
    refUI.velocitySpin->setValue(settingsData.velocity);
    refUI.densitySpin->setValue(settingsData.density);
    refUI.tempSpin->setValue(settingsData.temperature);
    refUI.tempUnitCombo->setCurrentText(settingsData.tempUnit);
    refUI.alphaSpin->setValue(settingsData.alpha);
    refUI.yawSpin->setValue(settingsData.yaw);

    // === 维度类型 ===
    bool nondim = (cleanQuotedText(settingsData.dimType) == "nondimensional");
...
{
    TurbulentDiffusionModelsData settingsData;

    settingsData.turbulenceModel = flowUI.turbGroup->checkedButton()
        ? normalizeQuoted(flowUI.turbGroup->checkedButton()->text())
        : "\"\"";

    settingsData.turbIntensity = flowUI.turbIntensitySpin->value();
    settingsData.turbMuRatio = flowUI.turbMuRatioSpin->value();


    settingsData.turbCompress = normalizeQuoted(flowUI.turbCompressCombo->currentText());
    settingsData.turbCond = normalizeQuoted(flowUI.turbCondCombo->currentText());

    settingsData.pr = flowUI.prSpin->value();
    settingsData.sc = flowUI.scSpin->value();
...
RawGridData structWidget::getRawGridData() const {
    RawGridData settingsData;

    // 所有字符串字段加上双引号
    settingsData.gridFormat = normalizeQuoted(rawUI.gridFormatCombo->currentText());
    settingsData.dataFormat = normalizeQuoted(rawUI.dataFormatCombo->currentText());

    // 布尔量仍使用 .true. / .false.
    settingsData.twodMode = rawUI.twodModeCheck->isChecked() ? ".true." : ".false.";
    settingsData.swapYZAxes = rawUI.swapYZAxesCheck->isChecked() ? ".true." : ".false.";

    settingsData.fieldviewPrecision = normalizeQuoted(rawUI.fieldviewPrecisionCombo->currentText());
    settingsData.patchLumping = normalizeQuoted(rawUI.patchLumpingCombo->currentText());
    settingsData.ignoreEulerNumber = rawUI.ignoreEulerCheck->isChecked() ? ".true." : ".false.";
...
{
    EqnData settingsData;

    // 所有 QString 类型字段都加上双引号
    settingsData.eqnType = normalizeQuoted(eqnUI.eqnTypeCombo->currentText());
    settingsData.artificialCompress = eqnUI.artificialCompressSpin->value();
    settingsData.viscousTerms = normalizeQuoted(eqnUI.viscousTermsCombo->currentText());
    settingsData.chemicalKinetics = normalizeQuoted(eqnUI.chemKineticsCombo->currentText());
    settingsData.thermalEnergyModel = normalizeQuoted(eqnUI.thermalModelCombo->currentText());
    settingsData.prandtlNumberMolecular = eqnUI.prandtlSpin->value();
    settingsData.schmidtNumber = eqnUI.schmidtSpin->value();
    settingsData.gasRadiation = normalizeQuoted(eqnUI.gasRadiationCombo->currentText());

    // bool 型逻辑字段仍然用 .true. / .false.
    settingsData.radUseImplLines = eqnUI.radUseImplCheck->isChecked() ? ".true." : ".false.";
    settingsData.multiComponentDiff = eqnUI.multiDiffCheck->isChecked() ? ".true." : ".false.";
    //settingsData.cpivMinFactor = eqnUI.cpivMinSpin->value();
    //settingsData.augmentKineticsLimiting = eqnUI.augmentKineticsCheck->isChecked() ? ".true." : ".false.";
    //settingsData.implicitRateLimiting = eqnUI.implicitRateCheck->isChecked() ? ".true." : ".false.";
...
{
    ForceMomentData settingsData;
    settingsData.areaReference   = fmUI.areaReferenceSpin->value();
    settingsData.xMomentLength   = fmUI.xMomentLengthSpin->value();
    settingsData.yMomentLength   = fmUI.yMomentLengthSpin->value();
    settingsData.xMomentCenter   = fmUI.xMomentCenterSpin->value();
    settingsData.yMomentCenter   = fmUI.yMomentCenterSpin->value();
    settingsData.zMomentCenter   = fmUI.zMomentCenterSpin->value();
    return settingsData;
}
...
{
    GridGenData settingsData;
    settingsData.nsrf = gridUI.nsrfSpin->value();
    settingsData.wake = gridUI.wakeSpin->value();
    settingsData.jmax = gridUI.jMaxSpin->value();
    settingsData.lesp = gridUI.lespSpin->value();
    settingsData.tesp = gridUI.tespSpin->value();
    settingsData.radi = gridUI.radiSpin->value();
    settingsData.re = gridUI.reSpin->value();
    settingsData.yPlus = gridUI.yPlusSpin->value();
    settingsData.genType = gridUI.genTypeBox->currentIndex();
...
{
    SpalartData settingsData;

    // 数值字段
    settingsData.turbInf = spalartUI.turbInfSpin->value();

    // 逻辑字段
    settingsData.daclesMariani = spalartUI.daclesCheck->isChecked() ? ".true." : ".false.";
    settingsData.sarc          = spalartUI.sarcCheck->isChecked() ? ".true." : ".false.";
    settingsData.ddes          = spalartUI.ddesCheck->isChecked() ? ".true." : ".false.";
...
{
    NonlinearSolverData settingsData;

    // === 基本参数 ===
    settingsData.timeAccuracy = normalizeQuoted(nonlinearUI.timeAccuracyCombo->currentText());
    settingsData.pseudoTimeStepping = normalizeQuoted(nonlinearUI.pseudoTimeSteppingCombo->currentText());
    settingsData.timeStepNondim = nonlinearUI.timeStepNondimSpin->value();
    settingsData.subiterations = nonlinearUI.subiterationsSpin->value();
    settingsData.temporalErrControl = nonlinearUI.temporalErrControlCheck->isChecked() ? ".true." : ".false.";
    settingsData.temporalErrFloor = nonlinearUI.temporalErrFloorSpin->value();

    // === 调度参数 ===
    settingsData.scheduleIteration1 = nonlinearUI.scheduleIteration1Spin->value();
    settingsData.scheduleIteration2 = nonlinearUI.scheduleIteration2Spin->value();
    settingsData.scheduleCfl1 = nonlinearUI.scheduleCfl1Spin->value();
    settingsData.scheduleCfl2 = nonlinearUI.scheduleCfl2Spin->value();
    settingsData.scheduleCflturb1 = nonlinearUI.scheduleCflturb1Spin->value();
    settingsData.scheduleCflturb2 = nonlinearUI.scheduleCflturb2Spin->value();

    // === 松弛与限制参数 ===
    settingsData.fAllowMinimumM = nonlinearUI.fAllowMinimumMSpin->value();
    settingsData.inviscRelaxFactor = nonlinearUI.inviscRelaxFactorSpin->value();
    settingsData.viscRelaxFactor = nonlinearUI.viscRelaxFactorSpin->value();
...
GlobalData structWidget::getGlobalData() const
{
    ...
    // 🟢 移动网格
    settingsData.movingGrid = globalUI.movingGridCheck->isChecked() ? ".true." : ".false.";
    settingsData.gridMotionOnly = globalUI.gridMotionOnlyCheck->isChecked() ? ".true." : ".false.";
    settingsData.gridMotionAndDCI = globalUI.gridMotionAndDCICheck->isChecked() ? ".true." : ".false.";
    settingsData.bodyMotionOnly = globalUI.bodyMotionOnlyCheck->isChecked() ? ".true." : ".false.";

    // 🟢 计时选项
    settingsData.timing = globalUI.timingCheck->isChecked() ? ".true." : ".false.";
    settingsData.timeMovingGrid = globalUI.timeMovingGridCheck->isChecked() ? ".true." : ".false.";

    // 🟢 输出频率
    settingsData.boundaryAnimFreq = globalUI.boundaryAnimSpin->value();
    settingsData.volumeAnimFreq = globalUI.volumeAnimSpin->value();
    settingsData.sliceFreq = globalUI.sliceFreqSpin->value();

    // 🟢 其他
    settingsData.recordCmd = globalUI.recordCmdCheck->isChecked() ? ".true." : ".false.";

    return settingsData;
}
VolumeOutputData structWidget::getVolumeOutputData() const
{
    VolumeOutputData settingsData;

    // === 输出格式 ===
    settingsData.exportTo = normalizeQuoted(volumeOutputUI.exportToCombo->currentText());


    // === 坐标 ===
    settingsData.x = volumeOutputUI.xCheck->isChecked() ? ".true." : ".false.";
    settingsData.y = volumeOutputUI.yCheck->isChecked() ? ".true." : ".false.";
    settingsData.z = volumeOutputUI.zCheck->isChecked() ? ".true." : ".false.";

    // === 基础变量 ===
    settingsData.primitive   = volumeOutputUI.primitiveCheck->isChecked() ? ".true." : ".false.";
    settingsData.rho         = volumeOutputUI.rhoCheck->isChecked() ? ".true." : ".false.";
    settingsData.u           = volumeOutputUI.uCheck->isChecked() ? ".true." : ".false.";
    settingsData.v           = volumeOutputUI.vCheck->isChecked() ? ".true." : ".false.";
    settingsData.w           = volumeOutputUI.wCheck->isChecked() ? ".true." : ".false.";
    settingsData.p           = volumeOutputUI.pCheck->isChecked() ? ".true." : ".false.";
    settingsData.entropy     = volumeOutputUI.entropyCheck->isChecked() ? ".true." : ".false.";
    settingsData.mach        = volumeOutputUI.machCheck->isChecked() ? ".true." : ".false.";
    settingsData.temperature = volumeOutputUI.temperatureCheck->isChecked() ? ".true." : ".false.";
    settingsData.iblank      = volumeOutputUI.iblankCheck->isChecked() ? ".true." : ".false.";
    settingsData.imesh       = volumeOutputUI.imeshCheck->isChecked() ? ".true." : ".false.";

    // === 涡量 / 散度 ===
    settingsData.vortMag     = volumeOutputUI.vortMagCheck->isChecked() ? ".true." : ".false.";
    settingsData.vortX       = volumeOutputUI.vortXCheck->isChecked() ? ".true." : ".false.";
    settingsData.vortY       = volumeOutputUI.vortYCheck->isChecked() ? ".true." : ".false.";
    settingsData.vortZ       = volumeOutputUI.vortZCheck->isChecked() ? ".true." : ".false.";
    settingsData.qCriterion  = volumeOutputUI.qCriterionCheck->isChecked() ? ".true." : ".false.";
    settingsData.divVel      = volumeOutputUI.divVelCheck->isChecked() ? ".true." : ".false.";

    // === 湍流脉动 ===
    settingsData.turbFluct   = volumeOutputUI.turbFluctCheck->isChecked() ? ".true." : ".false.";
    settingsData.uuPrime     = volumeOutputUI.uuPrimeCheck->isChecked() ? ".true." : ".false.";
    settingsData.vvPrime     = volumeOutputUI.vvPrimeCheck->isChecked() ? ".true." : ".false.";
    settingsData.wwPrime     = volumeOutputUI.wwPrimeCheck->isChecked() ? ".true." : ".false.";
