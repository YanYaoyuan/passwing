...
void airfoilDesign::splitAirfoilData(const QVector<QVector<double>> &airfoildata) {
    ...
    // 下表面：前缘之后的点
    for(int lowerIndex = i + 1; lowerIndex < length; lowerIndex++){
        temporaryAirfoilLower.append(airfoildata[lowerIndex]);
    }
...
void airfoilDesign::computeParameters(const QVector<double>&parameters){
    ...
    if(buildCSTSuccess){
        ...
        for(int i = 0; i < num + 1; i++){
            for(int j = 1; j < cstNum + 1; j++){
                upY = parameters[j - 1] * pow(airfoilX[i],0.5) * pow(1 - airfoilX[i],1) * nchoosek(cstNum - 1,j-1) * pow(1 - airfoilX[i],cstNum - j) * pow(airfoilX[i],j - 1) + upY;
            }
        ...
        for(int i = 0; i < num + 1; i++){
            for(int j = 1; j < cstNum + 1; j++){
                loY = parameters[ cstNum * 2 - j] * pow(airfoilX[i],0.5) * pow(1 - airfoilX[i],1) * nchoosek(cstNum - 1,j-1) * pow(1 - airfoilX[i],cstNum - j) * pow(airfoilX[i],j - 1) + loY;
            }
