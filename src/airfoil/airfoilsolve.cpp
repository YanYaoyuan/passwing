...
int airfoilSolve::solver(){
    ...
    for (int i = 0; i < step + 1; i++) {
        ...
        if (foil->lvconv) {
            ...
            for(int pointIndex = 0;pointIndex<airfoilData.size();pointIndex++){

                cpY.append(foil->cpi[pointIndex]);

            }
            for(int upperIndex = 0;upperIndex<foil->nbl[1];upperIndex++){
                upperTmp.append(foil->dstr[upperIndex][1]);
            }
            for(int lowerIndex = 0;lowerIndex<foil->nbl[2];lowerIndex++){
                lowerTmp.append(foil->dstr[lowerIndex][2]);
            }

            int len = foil->nbl[1] + foil->nbl[2] - airfoilData.size() - 2;

            for(int wakeIndex = 0;wakeIndex<len;wakeIndex++){
                wakeX.append(foil->x[wakeIndex + airfoilData.size() + 1]);
                wakeY.append(foil->y[wakeIndex + airfoilData.size() + 1]);
            }
