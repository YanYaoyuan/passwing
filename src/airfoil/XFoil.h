...
class XFoil {
 ...
 public:
  ...
  bool ExecQDES();
  bool initialize();
  bool initXFoilGeometry(int fn, double *fx, double *fy, double *fnx,                                //输入翼型大小、xy坐标、和一个等大小的法向n空矩阵1
                         double *fny);
  bool initXFoilAnalysis(double Re, double alpha, double Mach, double NCrit,
                         double XtrTop, double XtrBot, int reType, int maType,
                         bool bViscous, std::stringstream &outStream);                               //输入计算翼型所需的一些参数2
  ...
  void setBLInitialized(bool bInitialized) { lblini = bInitialized; }                              //标记边界层未初始化3

  double QInf() const { return qinf; }
  void setQInf(double v) { qinf = v; }                                                             //设置来流速度5

  double alpha() const { return alfa; }
  void setAlpha(double aoa) { alfa = aoa; }                                                        //设置迎角序列4

  double ClSpec() const { return clspec; }
  void setClSpec(double liftCoefficient) { clspec = liftCoefficient; }                              //指定升力系数
 ...
 public:
  ...
  double maxThickness;
  double locationThickness;
  double maxCamber;
  double locationCamber;
  ...
  */
  ...
  //	std::complex<double> zcoldw, dzte, chordz, zleold, zc, zc_cn, piq, cn, eiw;
