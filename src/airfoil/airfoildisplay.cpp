...
void airfoilDisplay::drawCheckCSTAirfoil(){
    ...
    if(!checkAirfoil.isEmpty()){
        ...
        const int cstCount = CSTNumBox->value();
        ...
        airfoilDesign designB(cstCount);
...
void airfoilDisplay::drawInterClResult(const airfoilData &result){
    if(!result.alphaData.isEmpty()){
        AirfoilPlotSeries *series = new AirfoilPlotSeries;
        for(int i = 0;i<result.alphaData.length();i++){
            series->append(result.alphaData[i],result.clData[i]);
        }
...
void airfoilDisplay::drawInterCdResult(const airfoilData &result){
    if(!result.alphaData.isEmpty()){
        AirfoilPlotSeries *series = new AirfoilPlotSeries;
        for(int i = 0;i<result.alphaData.length();i++){
            series->append(result.alphaData[i],result.cdData[i]);
        }
        dragInterChartA->addSeries(series);
        series->attachAxis(dragInterAxisXA);
        series->attachAxis(dragInterAxisYA);
...
void airfoilDisplay::drawInterKResult(const airfoilData &result){
    if(!result.alphaData.isEmpty()){
        AirfoilPlotSeries *series = new AirfoilPlotSeries;
        for(int i = 0;i<result.alphaData.length();i++){
            series->append(result.alphaData[i],result.clData[i] / result.cdData[i]);
        }
        dragInterChartA->addSeries(series);
        series->attachAxis(dragInterAxisXA);
        series->attachAxis(dragInterAxisYA);
...
void airfoilDisplay::drawInterPResult(const airfoilData &result){
    if(!result.alphaData.isEmpty()){
        AirfoilPlotSeries *series = new AirfoilPlotSeries;
        for(int i = 0;i<result.alphaData.length();i++){
            series->append(result.alphaData[i],pow(result.clData[i],1.5) / result.cdData[i]);
        }
        dragInterChartA->addSeries(series);
        series->attachAxis(dragInterAxisXA);
        series->attachAxis(dragInterAxisYA);
...
void airfoilDisplay::drawInterCmResult(const airfoilData &result){
    if(!result.alphaData.isEmpty()){
        AirfoilPlotSeries *series = new AirfoilPlotSeries;
        for(int i = 0;i<result.alphaData.length();i++){
            series->append(result.alphaData[i],result.cmData[i]);
        }
        dragInterChartA->addSeries(series);
        series->attachAxis(dragInterAxisXA);
        series->attachAxis(dragInterAxisYA);
...
void airfoilDisplay::drawInterRResult(const airfoilData &result){
    if(!result.alphaData.isEmpty()){
        AirfoilPlotSeries *series = new AirfoilPlotSeries;
        for(int i = 0;i<result.alphaData.length();i++){
            series->append(result.cdData[i],result.clData[i]);
        }
        dragInterChartA->addSeries(series);
        series->attachAxis(dragInterAxisXA);
        series->attachAxis(dragInterAxisYA);
...
void airfoilDisplay::addNewAirfoil(const QVector<QVector<double>>&airfoil){
    ...
    if(!airDesign->isFinshed()){
        ...
        */
        ...
        AirfoilPlotSeries *newSeriesA = new AirfoilPlotSeries;
        AirfoilPlotSeries *seriesB = new AirfoilPlotSeries;
        newSeriesA->setName(airfoilNameArray[airfoilIndex]);
        seriesB->setName(airfoilNameArray[airfoilIndex]);
        resultChartA->addSeries(newSeriesA);
        resultChartB->addSeries(seriesB);
        newSeriesA->attachAxis(resultAxisXA);
        newSeriesA->attachAxis(resultAxisYA);
        seriesB->attachAxis(resultAxisXB);
        seriesB->attachAxis(resultAxisYB);

        resultSeriesA.append(newSeriesA);
    ...
    for(int i = 0; i<threadNum;i++){
        ...
        if (step == 0) {
            index1 = i + (GaSetting.initialEliteNum / threadNum - workerNum + n) *
                             threadNum;
            chromosome = exp->initialCst[index1];
            design.buildAirfoilCurve(chromosome);
            ...
            chromosome = exp->chromosomeSequenceDec[index1];
            design.buildAirfoilCurve(chromosome);
...
void airfoilDisplay::updateDesignCSTPoint(){
    QVector<double>cstx;
    QVector<double>csty;
    const int cstCount = airfoilDesignModelArray[choiceIndex]->cstNum;
    cstNumArray[choiceIndex] = cstCount;

    cstx.append(0);
    csty.append(0.1);
    for(int i = 0;i < cstCount - 2;i++){
        cstx.append(static_cast<double>(i + 1) / (cstCount - 1));
        csty.append(0.15);
    }
    cstx.append(1);
    csty.append(0.1);
    ...
    csty.append(-0.1);
    for(int i = 0;i < cstCount - 2;i++){
        cstx.append(static_cast<double>(cstCount - i - 2) / (cstCount - 1));
        csty.append(-0.15);
    }
    cstx.append(0);
    csty.append(-0.1);
...
void airfoilDisplay::updateBlendingAirfoil(){
    if(blendingReadyA&&blendingReadyB){
        ...
        airfoilDesign design1(parameterCount);
        airfoilDesign design2(parameterCount);
        airfoilDesign design3(parameterCount);
...
int airfoilDisplay::findLeadingEdgeIndex(
    int index,
    const QVector<QVector<QVector<double>>>& airfoils)
{
    const auto& airfoil = airfoils[index];
