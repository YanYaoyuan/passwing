...
void airplaneDisplay::initialAirplaneMessage(){
    QGridLayout *messageLayout = new QGridLayout(messageBox);
    for(int i = 0;i<20;i++){
    ...
    messageLayout->addWidget(labelArray[0],0,0,1,1);
    messageLayout->addWidget(labelArray[1],1,0,1,1);
    messageLayout->addWidget(labelArray[2],2,0,1,1);
    messageLayout->addWidget(labelArray[3],3,0,1,1);
    messageLayout->addWidget(labelArray[4],4,0,1,1);
    messageLayout->addWidget(labelArray[5],5,0,1,1);
    messageLayout->addWidget(labelArray[6],6,0,1,1);
    messageLayout->addWidget(labelArray[7],7,0,1,1);

    messageLayout->addItem(verticalSpacer,8,0,1,1);
    messageLayout->addWidget(labelArray[8],9,0,1,1);
    messageLayout->addWidget(nameEdit,9,1,1,1);
    messageLayout->addWidget(saveButton,10,0,1,1);
    messageLayout->addWidget(cancelButton,10,1,1,1);
...
void airplaneDisplay::changeAirplaneDisplay(QTreeWidgetItem* item, int /*column*/){
    if (!item->parent()) { // 判断是否是子节点
        ...
        if(airplaneDataArray.length() > airplaneChoiceIndex){
            ...
            const int componentIndex = airplaneDataArray[airplaneChoiceIndex].compoentIndex[0];
            if(!wingDataArray.isEmpty())
                updateGLTextA(airplaneDataToString(wingDataArray[componentIndex]));
        }
