...
void propellerDisplay::initialStreamDialog(){
    ...
    QPushButton *streamSaveButton = new QPushButton(streamSettingDialog);
    QPushButton *streamCancelButton = new QPushButton(streamSettingDialog);
    for(int i = 0;i<4;i++){
    ...
    layout->addWidget(streamSaveButton,4,0,1,1);
    layout->addWidget(streamCancelButton,4,1,1,1);
    ...
    streamSaveButton->setText("确认");
    streamCancelButton->setText("取消");
    ...
    connect(streamSaveButton,&QPushButton::clicked,this,&propellerDisplay::showPropStreamLine);
    connect(streamCancelButton,&QPushButton::clicked,streamSettingDialog,&QDialog::hide);
...
void propellerDisplay::hidePressureContourView(){
    ...
    while ((currentActor = actors->GetNextActor())) {
        if (!currentActor->GetPickable()) {
            rendererE->RemoveActor(currentActor);
...
void propellerDisplay::updateAirfoilArray(const QVector<QVector<QVector<double>>>&airfoilArrayTmp,const QVector<QString>&incomingNames,const QVector<int>&cstNumArrayTmp){

    if(!airfoilArrayTmp.isEmpty() && !incomingNames.isEmpty()){
        ...
        propAirfoilNameArray = incomingNames;
        for(int i = 0;i<incomingNames.length();i++)
            propDesignAirfoilCombox->addItem(incomingNames[i]);
...
void propellerDisplay::updatePropDefineWidget(const wingDefinition& propData){
    ...
    if(!ned.gridA.isEmpty()){
        ...
        */
        ...
        profileMapper->SetInputData(profilePolyDataE);

        vtkSmartPointer<vtkActor> newActor = vtkSmartPointer<vtkActor>::New();
        newActor->SetMapper(profileMapper);
...
void propellerDisplay::updateView() {
    ...
    if (!propDesign->xyALower.isEmpty()) {
        ...
        // 创建新的映射器和演员
        vtkSmartPointer<vtkPolyDataMapper> profileMapper = vtkSmartPointer<vtkPolyDataMapper>::New();
        profileMapper->SetInputData(profilePolyData);
...
void propellerDisplay::CreateRotatedArray(vtkSmartPointer<vtkRenderer> ren, vtkSmartPointer<vtkPolyData> inputPolyData, double angle) {
    ...
    // 创建映射器和演员
    vtkSmartPointer<vtkPolyDataMapper> rotatedMapper = vtkSmartPointer<vtkPolyDataMapper>::New();
    rotatedMapper->SetInputData(transformedData);

    vtkSmartPointer<vtkActor> rotatedActor = vtkSmartPointer<vtkActor>::New();
    rotatedActor->SetMapper(rotatedMapper);

    vtkSmartPointer<vtkProperty> property = vtkSmartPointer<vtkProperty>::New();
    property->SetColor(1.0, 0.0, 0.0); // 可选：设置颜色或其他属性
    rotatedActor->SetProperty(property);
    rotatedActor->PickableOff();

    // 将演员添加到渲染器
    ren->AddActor(rotatedActor);
...
void propellerDisplay::initialDialogA(){
    ...
    QPushButton *dialogSaveButton = new QPushButton();
    ...
    hLayoutB->addWidget(dialogSaveButton);
    ...
    connect(dialogSaveButton,&QPushButton::clicked,this,&propellerDisplay::exportToChordTXT);
...
void propellerDisplay::initialDialogB(){
    ...
    hLayout->addWidget(dialogSaveButton);
    ...
    connect(dialogSaveButton,&QPushButton::clicked,this,&propellerDisplay::exportToTwistTXT);
...
void propellerDisplay::updatePressureContourView(const propellerVLM *ned){
    ...
    // 创建新的映射器和演员
    vtkSmartPointer<vtkPolyDataMapper> profileMapper = vtkSmartPointer<vtkPolyDataMapper>::New();
    profileMapper->SetInputData(profilePolyDataC);
...
void propellerDisplay::addPropData(){
    ...
    int b = rand() % 255;
    int g = rand() % 255;
    QColor randomColor(r, b, g);

    propColorArray.append(randomColor);
    QString colorStyle = QString("background-color: %1;").arg(randomColor.name());
    label1->setText("转速");
...
void propellerDisplay::changePropColor(const QColor& selectedColor){
    propColorArray[choiceIndex] = selectedColor;
...
double propellerDisplay::getFixVelocityTmp(const double v,const QVector<double>&x,const QVector<double>&samples){
    double tmp = 0;
    double minv = functionMath.minV(x);
    double maxv = functionMath.maxV(x);
    if(v>=minv&&v<=maxv){

        tmp = myMath::linearInterpolation(x,samples,v);
        return tmp;
    }
    else{
        return 0;
...
void propellerDisplay::clearChartView(QChart* chart, QVector<QLineSeries*>& seriesCollection) {
    // Step 1: 遍历所有的 QLineSeries 对象
    for (QLineSeries* series : seriesCollection) {
