...
void propellerBemt::initialAnalyse(const wingDefinition&definition,const propVLMSetting&settings){
    ...
    rpmArray = settings.rpmArray;
    vinfArray = settings.vinfArray;
    height = settings.height;
    propNum = definition.num;

    R = definition.spanW.last();
    for(int i = 0;i<definition.spanW.length() - 1;i++)
        spanArray.append(abs(definition.spanW[i] + definition.spanW[i + 1]) / 2);
    for(int i = 0;i<definition.spanW.length() - 1;i++)
        radiusArray.append(abs(definition.spanW[i] + definition.spanW[i + 1]) / 2);


    for(int i = 0;i<spanArray.length() - 1;i++){
        spanW.append(spanArray[i + 1] - spanArray[i]);
    }
    for(int i = 0;i<definition.chordLengthW.length() - 1;i++)
        chordArray.append((definition.chordLengthW[i] + definition.chordLengthW[i + 1]) / 2);

    //for(int i = 0;i<propData.chordLengthW.length() - 1;i++)
        //chordArray.append(propData.chordLengthW[i + 1]);


    for(int i = 0;i<definition.twistAngleW.length() - 1;i++)
        twistArray.append((definition.twistAngleW[i] + definition.twistAngleW[i + 1]) / 360.0 * M_PI);
    ...
    for(int i = 0;i<row;i++){
        for(int j = 0;j<columns;j++){
            ...
            for(int k = 0;k<spanW.length();k++){
                a = 0.1;
                b = 0.01;
                double sectionThrust = 0.0;
                double sectionTorque = 0.0;
                double v = 0.0;
                if(isModel){
                ...
                iterCompute(i,j,k,&sectionThrust,&sectionTorque,v);

                thrust += sectionThrust * spanW[k];
                torque += sectionTorque * spanW[k];

                dT.append(sectionThrust * spanW[k] / propNum);
                dQ.append(sectionTorque * spanW[k] / propNum / radiusArray[k]);
                //thrust += tmp1 * 0.072;
                //torque += tmp2 * 0.072;
