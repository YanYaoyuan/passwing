...
void propellerVLM::computeVLMMatrix(int n){
    ...
    for(int j = 0;j<N;j++){
        ...
        point3d surfaceNormal(Xn[j],Yn[j],Zn[j]);
        matrixb.append(-dotProduct(v,surfaceNormal));
...
*/
void propellerVLM::computeStreamLine(const int streamCaseIndex, const double distance,const double verticalOffset,const double t,const double dt){
    streamIndex = streamCaseIndex;
    //int rpmIndex = index%(rpmArray.length());
    int rpmIndex = streamCaseIndex % rpmArray.length();
...
void propellerVLM::computeVelocity(){
    ...
    for(int resultIndex = 0;resultIndex<matrixGammaArray.length();resultIndex++){
        ...
        for(int i = 0;i<N;i++){
            ...
            for(int j = 0;j<N + Ny * (wakeStep - 1) / 2;j++){
                vxt = vxt + vinxArray[resultIndex][i][j] * matrixGammaArray[resultIndex][j];
                vyt = vyt + vinyArray[resultIndex][i][j] * matrixGammaArray[resultIndex][j];
                vzt = vzt + vinzArray[resultIndex][i][j] * matrixGammaArray[resultIndex][j];
            }
            ...
            tmp1.append(point3d(vxt + vinfArray[resultIndex][i].x,vyt + vinfArray[resultIndex][i].y,vzt + vinfArray[resultIndex][i].z));
        }
        ...
        for(int q = 0;q<rpmArray.length();q++){
            ...
            for(int i = 0;i<Nx;i++){
                for(int j = 0;j<Ny;j++){
                    ...
                    //if(j > Ny / 2 - 1)
                        //c = -c;
                    double velocityX = streamVinfArray[alphaIndex][G].x;
                    double velocityY = streamVinfArray[alphaIndex][G].y;
                    double velocityZ = streamVinfArray[alphaIndex][G].z;


                    double a2 = (velocityY * c - velocityZ * b) * density;
                    double b2 = -(velocityZ * a - velocityX * c) * density;
                    double c2 = (velocityX * b - velocityY * a) * density;
...
void propellerVLM::addDrag(){
    ...
    if(isAddXfoilDrag){
        for(int operatingPointIndex = 0;operatingPointIndex < vinfArray.length();operatingPointIndex++){
            for(int i = 0;i<Ny;i++){
                xfoilSetting tmp1;
                ...
                double velocity = MVinfArray[operatingPointIndex][i];
                ...
                tmp1.xtrBot = 1;
                tmp1.designALPHA = 1;
                tmp1.designCL = spanLiftCoefficient[operatingPointIndex][i];
                tmp1.model = 1;
                tmp1.s_IterLim = 150;
        ...
        for(int i = 0;i<step;i++){
            int batchStartIndex = i * threadNum;
            startXfoilInThreadA(batchStartIndex,threadNum);
...
void propellerVLM::startXfoilInThreadA(const int startIndex,const int batchSize){
...
void propellerVLM::startXfoilInThreadB(const int startIndex,const int batchSize){
    QVector<QFuture<void>>futures;
    ...
    for(int i = 0;i<batchSize;i++){
        ...
        testSolve.append(solvers);
        airfoilDesign design(cstNum);
        design.buildAirfoilCurve(badCSTArray[startIndex + i],airfoilX);
        solvers->importAirfoil(design.newAirfoilData);
        solvers->refreshParaments(badSettingArray[startIndex + i]);
        QFuture<void>future = QtConcurrent::run([solvers](){
            solvers->solver();
        });
        futures.append(future);
...
void propellerVLM::smoothDrag() {
    ...
    for (int operatingPointIndex = 0; operatingPointIndex < vinfArray.length(); operatingPointIndex++) {
        ...
        for (int i = 0; i < len; i++) {
            int G = operatingPointIndex * len + i;

            // 比较 spanLiftCoefficient 和 spanLiftArray 的差异
            if (abs(spanLiftCoefficient[operatingPointIndex][i] - spanLiftArray[G]) > error) {
        ...
        QString txt;
        if(value <= 5){
            txt = "<font color='#388E3C'>状态(" + QString::number(operatingPointIndex + 1) + ")组阻力迭代收敛，阻力发散点占总比例" + QString::number(value) + "%</font>";
        }else if(value > 5&&value <= 15){
            txt = "<font color='#FBC02D'>状态(" + QString::number(operatingPointIndex + 1) + ")组阻力迭代收敛，阻力发散点占总比例" + QString::number(value) + "</font>%";
        }else{
            txt = "<font color='#C62828'>状态(" + QString::number(operatingPointIndex + 1) + ")组阻力迭代发散，阻力发散点占总比例" + QString::number(value) + "</font>%";
        }
...
void propellerVLM::smoothDragAgain(){
    ...
    for(int i = 0;i<step;i++){
        int batchStartIndex = i * minThreadNum;
        startXfoilInThreadB(batchStartIndex,minThreadNum);
...
void propellerVLM::generateWakeMesh(){
    ...
    for(int p = 0;p<vArray.length();p++){
        ...
        for(int q = 0;q<rpmArray.length();q++){
            ...
            const double dt = M_PI * 2 / azimuthalGridCount / omegaArray[q];
            ...
            for(int i = 1;i<wakeStep;i++){
                ...
                for(int j = 0;j<Ny + 1;j++){
                    ...
                    point3d wakePoint(realx,realy,realz);

                    tmp.append(wakePoint);
                }
                wakeTmp.append(tmp);
            }
...
void propellerVLM::generate3DModel(){
    ...
    for (int i = 0; i < num + 1; i++) {
        int sectionIndex = Ny; // 初始化index
        for (int k = 0; k < i; k++) {
            sectionIndex = sectionIndex - gridU[k]; // 确保index在内层循环中被正确设置
        }
        QVector<point3d> tmp;
        for (int j = Nx; j>=0; j--) {
            tmp.append(xyvvUpper[j][sectionIndex]);
        }
        for (int j = 1; j < Nx + 1; j++) {
            tmp.append(xyvvLower[j][sectionIndex]);
        }
...
*/
double propellerVLM::computeZ(const int sectionIndex, const double x, const double c){
    double z = 0;
    for(int i = 1;i<cstNum + 1;i++){
        z = realCst[sectionIndex][i - 1] * std::pow(x,0.5) * std::pow(1 - x,1) * nchoosek(cstNum - 1,i-1) * std::pow(1 - x,cstNum - i) * std::pow(x,i - 1) + z;
    }


    return z * c;
}
int propellerVLM::nchoosek(const int order,const int selectedCount) {
...
double propellerVLM::computeUpperZ(const int sectionIndex, const double x, const double c){
    double z = 0;
    for(int i = 1;i<cstNum + 1;i++){
        z = spanCstArray[sectionIndex][i - 1] * std::pow(x,0.5) * std::pow(1 - x,1) * nchoosek(cstNum - 1,i-1) * std::pow(1 - x,cstNum - i) * std::pow(x,i - 1) + z;
    }


    return (z + upperYte[sectionIndex] * x) * c;
}
double propellerVLM::computeLowerZ(const int sectionIndex, const double x, const double c){
    double z = 0;
    for(int i = 1;i<cstNum + 1;i++){
        z = spanCstArray[sectionIndex][cstNum * 2 - i] * std::pow(x,0.5) * std::pow(1 - x,1) * nchoosek(cstNum - 1,i-1) * std::pow(1 - x,cstNum - i) * std::pow(x,i - 1) + z;
    }
    return (z + lowerYte[sectionIndex] * x) * c;
}
...
QVector<double> propellerVLM::calculateBendingMoment(const QVector<double>& spanForce, const QVector<double>& spanLocations) {
    QVector<double>tmp;
    int len = spanLocations.length();
    for(int i = 0;i<len;i++){
        double tmp1 = 0;
        for(int j = 0;j<len - i;j++){
            tmp1 += (spanYt[len - i - 1] - spanLocations[j]) * spanForce[j];
        }
        tmp.append(tmp1);
    }
    tmp.append(0);
...
double propellerVLM::calculateTorque(const QVector<double>& spanForce, const QVector<double>& spanLocations) {
    int len = spanLocations.length();
    double tmp1 = 0;
    for(int j = 0;j<len;j++){
        tmp1 += (-spanLocations[j]) * spanForce[j];
    }
    return tmp1;
}
...
point3d propellerVLM::rungeKutta4(const point3d& position,const double deltaTime,const int streamlineIndex){

    point3d k1 = calculateVelocity(position,streamlineIndex);
    point3d position1(position.x + k1.x * (deltaTime / 2),position.y + k1.y * (deltaTime / 2),position.z + k1.z * (deltaTime / 2));
    point3d k2 = calculateVelocity(position1,streamlineIndex);
    point3d position2(position.x + k2.x * (deltaTime / 2),position.y + k2.y * (deltaTime / 2),position.z + k2.z * (deltaTime / 2));
    point3d k3 = calculateVelocity(position2,streamlineIndex);
    point3d position3(position.x + k3.x * deltaTime,position.y + k3.y * deltaTime,position.z + k3.z * deltaTime);
    point3d k4 = calculateVelocity(position3,streamlineIndex);
...
point3d propellerVLM::calculateVelocity(const point3d& position,const int streamlineIndex){
    ...
    double vm = -yt[streamlineIndex] * omegaArray[rpmIndex];
    ...
    return point3d(vxt + vm * cos(thate),vyt + vm * sin(thate),vzt + vinfArray[streamIndex][streamlineIndex].z);
}
