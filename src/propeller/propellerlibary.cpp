...
void propellerLibary::findNearestPoints(const QVector<QVector<double>>& samples, double target_x, QVector<QVector<double>>& nearestPoints) {
    // Vectors to store distances and indices
       QVector<double> distances(samples.size());
       QVector<int> indices(samples.size());

       for (int i = 0; i < samples.size(); ++i) {
           double x_value = samples[i][0]; // Assume the first column is x
       ...
       for (int i = 0; i < 3; ++i) {
           nearestPoints.append(samples[indices[i]]);
       }
}
...
double propellerLibary::threePointInterpolation(const QVector<QVector<double>>& samples, double target_x, int column) {
    QVector<QVector<double>> nearestPoints;

    // Find nearest points for interpolation
    findNearestPoints(samples, target_x, nearestPoints);
